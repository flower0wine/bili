...
export async function parseResponse<T>(
  promiseOrFn: Promise<AxiosResponse<Http.ApiResponse<T>>> | (() => Promise<AxiosResponse<Http.ApiResponse<T>>>)
...
export async function parseResponse<T>(
  ...
  try {
    // 支持直接传入Promise或函数
    const response = typeof promiseOrFn === 'function' ? await promiseOrFn() : await promiseOrFn;
 ...
 * ```
 */
export async function parseResponses<T extends readonly unknown[]>(
  responses: {
    [K in keyof T]: Promise<AxiosResponse<Http.ApiResponse<T[K]>>> | (() => Promise<AxiosResponse<Http.ApiResponse<T[K]>>>)
  }
): Promise<{
  [K in keyof T]: [T[K] | null, Http.ErrorResponse | null]
}> {
  const promises = responses.map((response): Promise<[unknown, Http.ErrorResponse | null]> =>
 ...
 * 检查响应是否成功
 */
export function isSuccess<T>(response: [T | null, Http.ErrorResponse | null]): response is [T, null] {
 ...
 * 检查响应是否失败
 */
export function isError<T>(response: [T | null, Http.ErrorResponse | null]): response is [null, Http.ErrorResponse] {
  const [data, error] = response;
  return data === null && error !== null;
}
