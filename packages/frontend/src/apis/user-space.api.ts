...
export const userSpaceApi = {
  ...
  // 分页获取所有用户的最新用户空间数据
  getAllLatestUserSpaceData: async (params?: {
    page?: number;
    limit?: number;
  }): Promise<
    AxiosResponse<Http.ApiResponse<PaginatedResponse<UserSpace.UserSpaceVO>>>
  > => {
    return api.get("/v1/user-space/latest", { params });
  },
  ...
  getUserSpaceDataByLevelRange: async (params?: {
    ...
    AxiosResponse<Http.ApiResponse<PaginatedResponse<UserSpace.UserSpaceVO>>>
  > => {
    return api.get("/v1/user-space/level-range", { params });
  },

  // 根据认证状态查询用户空间数据
  getVerifiedUserSpaceData: async (params?: {
    page?: number;
    limit?: number;
  }): Promise<
    AxiosResponse<Http.ApiResponse<PaginatedResponse<UserSpace.UserSpaceVO>>>
  > => {
    return api.get("/v1/user-space/verified", { params });
  },

  // 搜索用户空间数据
  searchUserSpaceData: async (
    keyword: string,
    params?: { page?: number; limit?: number },
  ): Promise<
    AxiosResponse<Http.ApiResponse<PaginatedResponse<UserSpace.UserSpaceVO>>>
  > => {
    return api.get("/v1/user-space/search", {
      params: { keyword, ...params },
    });
  },

  // 获取用户空间数据统计信息
  getUserSpaceStats: async (
    mid?: number,
  ): Promise<
    AxiosResponse<
      Http.ApiResponse<{
